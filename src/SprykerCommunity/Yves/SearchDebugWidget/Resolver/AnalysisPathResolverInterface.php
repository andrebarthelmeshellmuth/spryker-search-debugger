...
interface AnalysisPathResolverInterface
{
     ...
     * @param int $endOffset
     *
     * @return array<int, array{text: string, operation: string|null}>|null Null when the target
     *   offset isn't found in the last analysis stage at all (e.g. stale offsets from a re-indexed
     *   document). The first entry's `operation` is always null (it's the origin, nothing produced it).
     */
